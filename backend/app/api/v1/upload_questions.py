...
# TTL 1800s：转录任务通常 1~3 分钟，TTL 足够前端轮询；过期后轮询返回 not_found
_upload_cache: TTLCache = TTLCache(maxsize=200, ttl=1800)
...
async def _run_transcription_task(
    ...
    try:
        ...
        _upload_cache[task_id] = {"status": "failed", "error": "处理超时，请稍后重试", "user_id": user_id}
    except Exception as exc:
        logger.exception("转录任务 %s 失败: %s", task_id, exc)
        # 用户可读的错误信息：HTTPException 取 detail，其余取 str
        detail = exc.detail if isinstance(exc, HTTPException) else str(exc)
        _upload_cache[task_id] = {"status": "failed", "error": detail or "转录失败，请重试", "user_id": user_id}
...
async def _do_transcription(
    ...
    所有文件均未转录出有效题目才整体失败。
    """
    _upload_cache[task_id] = {"status": "processing", "user_id": user_id}

    all_entries: list[dict] = []
    saved_total = 0
    for file_data, filename in files:
        try:
            entries, saved = await _transcribe_one_file(user_id, file_data, filename, meta)
...
async def upload_questions(
    ...
    _upload_cache[task_id] = {"status": "pending", "user_id": current_user.id}
    meta = {
        "grade": grade,
        "subject": subject,
        "semester": semester,
    ...
    run_async_in_background(
        _run_transcription_task(task_id, current_user.id, prepared, meta)
    )
...
async def get_upload_result(
    ...
    cached = _upload_cache.get(task_id)
